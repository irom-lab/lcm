"""LCM type definitions
This file automatically generated by lcm.
DO NOT MODIFY BY HAND!!!!
"""

try:
    import cStringIO.StringIO as BytesIO
except ImportError:
    from io import BytesIO
import struct

class arrays_t(object):
    __slots__ = ["arr"]

    __typenames__ = ["int8_t"]

    __dimensions__ = [[10]]

    dim = 10

    def __init__(self):
        self.arr = [ 0 for dim0 in range(10) ]

    def encode(self):
        buf = BytesIO()
        buf.write(arrays_t._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack('>10b', *self.arr[:10]))

    def decode(data):
        if hasattr(data, 'read'):
            buf = data
        else:
            buf = BytesIO(data)
        if buf.read(8) != arrays_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return arrays_t._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = arrays_t()
        self.arr = struct.unpack('>10b', buf.read(10))
        return self
    _decode_one = staticmethod(_decode_one)

    def _get_hash_recursive(parents):
        if arrays_t in parents: return 0
        tmphash = (0x3f2258e55559faf8) & 0xffffffffffffffff
        tmphash  = (((tmphash<<1)&0xffffffffffffffff) + (tmphash>>63)) & 0xffffffffffffffff
        return tmphash
    _get_hash_recursive = staticmethod(_get_hash_recursive)
    _packed_fingerprint = None

    def _get_packed_fingerprint():
        if arrays_t._packed_fingerprint is None:
            arrays_t._packed_fingerprint = struct.pack(">Q", arrays_t._get_hash_recursive([]))
        return arrays_t._packed_fingerprint
    _get_packed_fingerprint = staticmethod(_get_packed_fingerprint)

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", arrays_t._get_packed_fingerprint())[0]

