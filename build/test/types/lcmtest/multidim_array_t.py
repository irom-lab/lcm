...
class multidim_array_t(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(multidim_array_t._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != multidim_array_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return multidim_array_t._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = multidim_array_t()
    ...
    def _get_hash_recursive(parents):
        if multidim_array_t in parents: return 0
    ...
    def _get_packed_fingerprint():
        if multidim_array_t._packed_fingerprint is None:
            multidim_array_t._packed_fingerprint = struct.pack(">Q", multidim_array_t._get_hash_recursive([]))
        return multidim_array_t._packed_fingerprint
    _get_packed_fingerprint = staticmethod(_get_packed_fingerprint)

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", multidim_array_t._get_packed_fingerprint())[0]
