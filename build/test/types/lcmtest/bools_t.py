...
class bools_t(object):
    ...
    def _decode_one(buf):
        ...
        self.fixed_array = [bool(x) for x in struct.unpack('>3b', buf.read(3))]
        self.num_a, self.num_b = struct.unpack(">bb", buf.read(2))
        self.one_dim_array = [bool(x) for x in struct.unpack('>%db' % self.num_a, buf.read(self.num_a))]
        self.two_dim_array = []
        for i0 in range(self.num_a):
            self.two_dim_array.append([bool(x) for x in struct.unpack('>%db' % self.num_b, buf.read(self.num_b))])
        return self
    ...
    def get_hash(self):
        ...
        return struct.unpack(">Q", bools_t._get_packed_fingerprint())[0]
